...
IUSE="notification test +X +encryption +keychain +sound"
...
RDEPEND="
  ...
  sound? ( dev-libs/miniaudio )
  encryption? ( app-crypt/qca )
...
#PATCHES=("${FILESDIR}/${P}-fix-cmake-for-qt6-6.10.patch")
...
src_configure() {
  local mycmakeargs=(
    ...
    -DWITH_X11=$(usex X)
    -DWITH_QCA_ENCRYPTION=$(usex encryption)
    -DWITH_KEYCHAIN=$(usex keychain)
    -DWITH_AUDIO=$(usex sound)
  )

  if use sound; then
    mycmakeargs+=(-DMINIAUDIO_INCLUDE_DIR="${EPREFIX}/usr/include/miniaudio")
  fi
