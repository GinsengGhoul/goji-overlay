...
SLOT="0"
KEYWORDS="-* ~amd64"
IUSE="opencl pulseaudio wayland +zoom-symlink +packagedQt"
...
src_install() {
  ...
  if use packagedQt; then
    doins -r Qt

    # remove QT libraries that don't resolve correctly
    rm -rf "${D}"/opt/zoom/Qt/plugins/egldeviceintegrations \
      "${D}"/opt/zoom/Qt/plugins/platforms/libqeglfs.so \
      "${D}"/opt/zoom/Qt/qml/Qt/labs/animation \
      "${D}"/opt/zoom/Qt/qml/Qt/labs/lottieqt \
      "${D}"/opt/zoom/Qt/qml/Qt/labs/qmlmodels \
      "${D}"/opt/zoom/Qt/qml/Qt/labs/settings \
      "${D}"/opt/zoom/Qt/qml/Qt/labs/sharedimage \
      "${D}"/opt/zoom/Qt/qml/Qt/labs/wavefrontmesh \
      "${D}"/opt/zoom/Qt/qml/Qt/test/controls \
      "${D}"/opt/zoom/Qt/qml/QtNetwork \
      "${D}"/opt/zoom/Qt/qml/QtQml/XmlListModel \
      "${D}"/opt/zoom/Qt/qml/QtQuick/LocalStorage \
      "${D}"/opt/zoom/Qt/qml/QtQuick/Particles \
      "${D}"/opt/zoom/Qt/qml/QtQuick/VectorImage
  fi
}
